###Data Resources:###
##https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/CXDJM0##

##Data recurrent##
#requires STATA 14 and some command fixing#
...
if (file.exists("data/0422analysis.dta")) {
  df <- read.dta13("data/0422analysis.dta")
} else {
  stop("Error: data/0422analysis.dta not found. Please make sure the working directory is correct.")
}
...
stargazer(under_reg0, under_reg1, under_reg2, type = "latex", single.row = TRUE, no.space = TRUE, column.sep.width = "0.01pt", font.size = "scriptsize", out="output/China_village_understanding_reg.tex")
summary(under_reg0)
...
st(df_table, summ=c('mean(x)','median(x)','max(x)', 'min(x)'), summ.names = c("Mean", "Median", "Max", "Min"), group = "region", out = "latex", file="output/China_village_summary_table.tex")
...
vars
stargazer(logit_reg_1, type = "text", keep = vars, omit = vars, out="output/China_village_logit_reg_1.txt")
...
png("output/CV_Lasso_Path_IS.png")
par(mfrow=c(1,2))
...
png("output/CV_Prediction_Hist_IS.png")
...
#R2 IS#
1 - logit_dev(df$takeup_survey, predict_IS_cv)/logit_dev(df$takeup_survey, mean(df$takeup_survey))
...
#Plots
#Plots
png("output/CV_Lasso_Path_OOS.png")
par(mfrow=c(1,2))
plot(cv.logit1$gamlr, main = "Lasso Regularization path")
...
set.seed(0)
randomforest_reg_IS <- ranger(takeup_survey ~ ., data = df_rf, write.forest = TRUE, num.trees = 300, min.node.size = 100, importance = "impurity", probability = TRUE)
randomforest_pred_IS <- predict(randomforest_reg_IS, df_rf, type = "response")$predict
png("output/RF_Importance_IS.png")
par(mfrow = c(1,2))
par(mar=c(12,4,1,1))
...
actual_correct_rate_rf_IS * 100

#IS RandomForest Prediction distribution
#IS RandomForest Prediction distribution
png("output/RF_Pred_Dist_IS.png")
par(mfrow=c(1,2))
...
#R2 IS RF#
1 - logit_dev(df_rf$takeup_survey, randomforest_pred_organize_IS)/logit_dev(df_rf$takeup_survey, mean(df_rf$takeup_survey))

#OUt-Of-Sample Random Forest#
#Data Splitting
set.seed(0)
test <- sample.int(nrow(df_rf), nrow(df_rf)/2)

#OOS Random Forest
randomforest_reg_OOS <- ranger(takeup_survey ~ ., data = df_rf[-test, ], write.forest = TRUE, num.trees = 300, min.node.size = 100, importance = "impurity", probability = TRUE)
randomforest_pred_OOS <- predict(randomforest_reg_OOS, df_rf[test, ], type = "response")$predict
Y_OOS <- df$takeup_survey[test]

png("output/RF_Importance_OOS.png")
par(mar=c(12,4,1,1))
...
#R2 OOS RF
1 - logit_dev(df$takeup_survey[test], randomforest_pred_organize_OOS)/logit_dev(df$takeup_survey[test], mean(df$takeup_survey[-test]))

#OOS ROC Random Forest
#OOS RandomForest Prediction distribution
png("output/RF_ROC_OOS.png")
#OOS ROC Random Forest
#OOS RandomForest Prediction distribution
par(mfrow=c(1,2))
...
for (k in 1:K) {
  ...
  store_results$LASSO[k] <- -sum(df$takeup_survey[!train] * log(yhat.lin) + (1 - df$takeup_survey[!train]) * log(1 - yhat.lin))
  #random forest
  ...
  store_results$RF[k] <- -sum(df_rf$takeup_survey[!train] * log(yhat.rf) + (1 - df_rf$takeup_survey[!train]) * log(1 - yhat.rf))
}

png("output/Model_Comparison_Boxplot.png")
